...
"After Glacial modifications."
    ...
    if gridtype == 'xy':
        ...
        net_Acc_EHEP = appice.createVariable("Acc_EHEP","f8",("y","x"))
        net_lat[:,:] = lat
        net_lon[:,:] = lon
    else:
...
def apply_watermask(Ehep,lat,lon,x=None,y=None):
    ...
    if gridtype == 'xy':
        mask = np.reshape(mask,(y,x))
        alt_Ehep = np.zeros((y,x))
    else:
        mask = np.reshape(mask,(len(lat),len(lon)))
        alt_Ehep = np.zeros((len(lat),len(lon)))
...
"After water/land mask modifications."
    ...
    if gridtype == 'xy':
        ...
        net_Acc_EHEP = appmask.createVariable("Acc_EHEP","f8",("y","x"))
        ...
        net_Acc_EHEP = appmask.createVariable("Acc_EHEP","f8",("lat","lon"))
...
"After forest modifications."
    ...
    if gridtype == 'xy':
        ...
        net_Acc_EHEP = appforest.createVariable("Acc_EHEP","f8",("y","x"))
        ...
        net_Acc_EHEP = appforest.createVariable("Acc_EHEP","f8",("lat","lon"))
...
def apply_orography(Ehep,lat,lon,x=None,y=None):
    ...
    alt_Ehep = Ehep * height_int * std_int
...
"After orography modifications."
    ...
    if gridtype == 'xy':
        ...
        net_Acc_EHEP = apporo.createVariable("Acc_EHEP","f8",("y","x"))
        ...
        net_Acc_EHEP = apporo.createVariable("Acc_EHEP","f8",("lat","lon"))
...
"Accesible EHEP and the vegetation data."
    ...
    if gridtype == 'xy':
        ...
        net_Acc_EHEP = appcap.createVariable("Acc_EHEP","f8",("y","x"))
        ...
        net_Acc_EHEP = appcap.createVariable("Acc_EHEP","f8",("lat","lon"))
        net_watermask = appcap.createVariable("watermask","f8",("y","x"))
        net_lat[:] = lat
        net_lon[:] = lon
...
def preprocess(EHEP,lat,lon,x=None,y=None):
    ...
    if gridtype == 'xy':
        ...
        if starting_dist == 'uniform':
            ...
            density[:,:] = gauß * mask
        elif starting_dist == 'square':
#           density[:,:] = 2. * mask[:,:]
            density[int(y/2)-5:int(y/2)+5,int(x/2)-5:int(x/2)+5] = 2.
    else:
...
"the mobility model."
    ...
    net_Acc_EHEP[:,:] = EHEP[:,:]
    net_Acc_EHEP_num[:,:] = EHEP[:,:] * net_area[:,:]/100.
    net_watermask[:,:] = mask[:,:]
...
if __name__ == "__main__":
    ...
    path_input = "/data/sfb806/human_mobility_model/konstantin/results/HEP_Heinrich/new_data_Gerd/ehep_auri-13_all_vHEwc_lr2_none_1000.nc"
    path_ice = "/data/sfb806/human_mobility_model/data/climate/pmip3_modified/lgm/ice_30s_exeu.nc"
    path_watermask = "/data/sfb806/human_mobility_model/data/climate/pmip3_modified/landmask.snipexeu.nc"
    path_veg = "/data/sfb806/human_mobility_model/data/climate/pmip3_modified/lgm/landuse_lgm_5min_exeu.nc"
    path_topo = "/data/sfb806/human_mobility_model/data/climate/topo_today.nc"
    path_output = "/data/sfb806/human_mobility_model/christian/"
    
    # Output file names for intermediate steps and end result
    file_out_glac = "Acc_EHEP_glac.nc"
    file_out_watermask ="Acc_EHEP_watermask.nc"
    file_out_forest = "Acc_EHEP_forest.nc"
    file_out_oro = "Acc_EHEP_oro.nc"
    file_out_capa = "Acc_EHEP_capacity.nc"
    file_out_mobil = "Acc_HEP_test_linear_closed_block_0.5deg.nc"
    
    # Miscellaneous settings for the calculations
    save_interm = False
    starting_dist = 'square'
    gridtype = 'latlon'
    ...
    if gridtype == 'xy':
        ...
        ### REMOVE THIS FOR OTHER APPLICATIONS! ONLY USED IN IDEALIZED!
        #mask = np.array(inputdata.variables[var_name_watermask_mask][:][:])
