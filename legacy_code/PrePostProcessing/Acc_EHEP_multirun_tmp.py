...
if __name__ == "__main__":
    ...
    input_file = "HEP_predef_0.5_deg.nc"
    path_input = "/data/sfb806/human_mobility_model/christian/200yrLGM/output/" + input_file
    path_ice = "/data/sfb806/human_mobility_model/data/climate/ice_30s_boolean_eu.nc"
    ...
    path_output = "/data/sfb806/human_mobility_model/christian/200yrLGM/output/"
    
    # Output file name
    file_out_Acc = "Acc_HEP_200yr_LGM_monthly.nc"
    file_out_Preproc = "Acc_HEP_200yr_LGM_monthly_preproc_2.nc"
    ...
    var_name_ice_cover = "ice_cover_boolean"
    ...
    if gridtype == 'xy':
        ...
        new_EHEP = np.zeros((runs,y,x))
        mean_EHEP = np.zeros((y,x))
        if switch_glac is True:
            glac_EHEP = np.zeros((runs,y,x))
            glac_mask = np.zeros((y,x))
        if switch_water is True:
            water_EHEP = np.zeros((runs,y,x))
            water_mask = np.zeros((y,x))
        if switch_forest is True:
            forest_EHEP = np.zeros((runs,y,x))
            forest_mask = np.zeros((y,x))
        if switch_oro is True:
            oro_EHEP = np.zeros((runs,y,x))
            ele_mask = np.zeros((y,x))
            std_mask = np.zeros((y,x))
        if switch_tech is True:
            tech_EHEP = np.zeros((runs,y,x))
            tech_mask = np.zeros((y,x))
    elif gridtype == 'latlon':
        x = None
        y = None
        lat = np.array(inputdata.variables[var_name_lat][:])
        ...
        lon_flat = lon[:]
        runs = len(inputdata.dimensions[var_name_runs])
        new_EHEP = np.zeros((runs,len(lat),len(lon)))
        mean_EHEP = np.zeros((len(lat),len(lon)))
        if switch_glac is True:
            glac_EHEP = np.zeros((runs,len(lat),len(lon)))
            glac_mask = np.zeros((len(lat),len(lon)))
        if switch_water is True:
            water_EHEP = np.zeros((runs,len(lat),len(lon)))
            water_mask = np.zeros((len(lat),len(lon)))
        if switch_forest is True:
            forest_EHEP = np.zeros((runs,len(lat),len(lon)))
            forest_mask = np.zeros((len(lat),len(lon)))
        if switch_oro is True:
            oro_EHEP = np.zeros((runs,len(lat),len(lon)))
            ele_mask = np.zeros((len(lat),len(lon)))
            std_mask = np.zeros((len(lat),len(lon)))
        if switch_tech is True:
            tech_EHEP = np.zeros((runs,len(lat),len(lon)))
            tech_mask = np.zeros((len(lat),len(lon)))
    else:
